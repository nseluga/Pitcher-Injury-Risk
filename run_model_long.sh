...
3. Write a brainstorm list of 8-10 new approaches to
   ...
   claude_instructions.md). REQUIRED before picking an idea.

4. Pick the highest-expected-value idea not yet attempted. If
   improvement_progress.json does not exist, create it with baseline
...
8. Measure C-index and IBS delta. Note interpretable HRs or survival curves.

9. If the change hurt metrics, revert it:
   git checkout notebooks/07_survival_models.ipynb src/models/survival_models.py

10. Log results in docs/model_improvement_log.md.

11. Update .scratch/improvement_progress.json. Increment
    consecutive_non_improvements if C-index delta < 0.005.
